...
ServerStats simulateQueue(double lambda, double mu, int numServers, int numCustomers, 
                         bool enableReneging, double fixedWaitThreshold, 
    ...
    while (completedCustomers + abandonedCustomers < numCustomers && !eventQueue.empty()) {
        ...
        if (event.type == Event::ARRIVAL) {
            ...
            if (enableReneging) {
                // Set fixed maximum wait time for abandonment (5 minutes = 0.0833 hours)
                newCustomer.maximumWaitTime = fixedWaitThreshold;
            } else {
    ...
    if (printDetails) {
        outFile << "======== COFFEE SHOP M/M/" << numServers << (enableReneging ? " QUEUE WITH FIXED THRESHOLD RENEGING" : " QUEUE") << " SIMULATION RESULTS ========" << std::endl;
        ...
        if (enableReneging) {
            outFile << "  - Fixed Abandonment Threshold: " << fixedWaitThreshold << " hour(s) (" 
                    << fixedWaitThreshold * 60 << " minutes)" << std::endl;
        }
...
int main() {
    ...
    const double FIXED_WAIT_THRESHOLD = 5.0 / 60.0;   // 5 minutes converted to hours (0.0833)
    ...
    // Simulation with fixed threshold reneging
    std::ofstream outFileReneging("coffee_shop_simulation_with_fixed_reneging_results.txt");
    if (!outFileReneging) {
        std::cerr << "Error opening output file!" << std::endl;
        return 1;
    }
    ...
    outFileReneging << "   COFFEE SHOP QUEUE SIMULATION WITH FIXED THRESHOLD RENEGING" << std::endl;
    outFileReneging << "==============================================" << std::endl << std::endl;

    std::cout << "Running baseline simulation with fixed threshold reneging..." << std::endl;
    ServerStats baselineStatsReneging = simulateQueue(LAMBDA, MU, 1, NUM_CUSTOMERS, true, FIXED_WAIT_THRESHOLD, true, outFileReneging, gen);
    
    std::cout << "Running service rate experiments with fixed threshold reneging..." << std::endl;
    ServerStats fasterStatsReneging = simulateQueue(LAMBDA, 20.0, 1, NUM_CUSTOMERS, true, FIXED_WAIT_THRESHOLD, true, outFileReneging, gen);
    ServerStats slowerStatsReneging = simulateQueue(LAMBDA, 12.0, 1, NUM_CUSTOMERS, true, FIXED_WAIT_THRESHOLD, true, outFileReneging, gen);
    
    std::cout << "Running multiple server simulation with fixed threshold reneging..." << std::endl;
    ServerStats multiServerStatsReneging = simulateQueue(LAMBDA, MU, 2, NUM_CUSTOMERS, true, FIXED_WAIT_THRESHOLD, true, outFileReneging, gen);
    ...
    std::cout << "- coffee_shop_simulation_with_fixed_reneging_results.txt (with fixed threshold reneging)" << std::endl;
